...
def scidx_from_unitcell(frame, j=0,T=[0,0,0], kmesh=None):
    ...
    assert all([t>=0 for t in T]), "T must be non-negative" # T must be positive for the logic 
    if kmesh is None: 
...
def scidx_to_mic_translation(frame, I=0, J=0, kmesh=[1,1,1], epsilon=1e-10):
    ...
    if J>=len(superframe):
        # print(J)
        J = J % len(superframe)
        warnings.warn("J is greater than the number of atoms in the supercell. Mapping J to J % len(superframe)")
    assert I<len(superframe) and J<len(superframe), "I and J must be less than the number of atoms in the supercell"
    ...
    return np.floor(cell_inv@d + epsilon).astype(int) # adding noise to avoid numerical issues
    # FIXME this sometimes returns [-3,-2,0] for [0 1 2] which is not correct as the output is not "Minimal" per se
