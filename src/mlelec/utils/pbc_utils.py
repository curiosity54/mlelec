...
def inverse_bloch_sum(dataset, matrix, A, cutoff):
    ...
    T_list = np.linalg.solve(
        dataset.cells[A].lattice_vectors().T,
        dataset.cells[A].get_lattice_Ls(rcut=cutoff / Bohr, dimension=dimension).T,
    ).T
    assert np.linalg.norm(T_list - np.round(T_list)) < 1e-9, np.linalg.norm(
        Ts - np.round(Ts)
    )
    ...
    if isinstance(matrix, np.ndarray):
        ...
        assert isinstance(
            matrix, torch.Tensor
        ), f"Matrix must be np.ndarray or torch.Tensor, but it's {type(matrix)}"
    ...
    if phys_cutoff < cutoff:
        warnings.warn(
            f"Structure {A} is not large enough for the selected cutoff. Real space target computed with cutoff of {phys_cutoff:.2f} Angstrom"
        )
        cutoff = phys_cutoff
...
def matrix_to_blocks(
    ...
    block_builder = TensorBuilder(
        key_names, sample_names, component_names, property_names, device=device
    )
    ...
    for A, frame in enumerate(dataset.structures):  # Loop over frames
        if not dataset.is_molecule:
            if cutoff_was_none:
                ...
                warnings.warn(
                    f"Automatic choice of the cutoff for structure {A}. rcut = {cutoff:.2f} Angstrom"
                )
            if matrix is None:
                if target.lower() == "fock":
                    if dataset.fock_realspace is None:
                        matrices = inverse_bloch_sum(
                            dataset, dataset.fock_kspace[A], A, cutoff
                        )
                    else:
                    ...
                    if dataset.overlap_realspace is None:
                        matrices = inverse_bloch_sum(
                            dataset, dataset.overlap_kspace[A], A, cutoff
                        )
                    else:
        ...
        for iii, T in enumerate(matrices):
            ...
            for i, ai in enumerate(frame.numbers):
                ...
                for j, aj in enumerate(frame.numbers):
                    ...
                    ij_distance = np.linalg.norm(
                        frame.cell.array.T @ np.array(T)
...
def move_cell_shifts_to_keys(blocks):
    ...
    for key, block in blocks.items():
        ...
        for T in translations:
            block_view = block.samples.view(
                ["cell_shift_a", "cell_shift_b", "cell_shift_c"]
            ).values
            idx = torch.where(
                torch.all(
                    torch.isclose(block_view, torch.tensor([T[0], T[1], T[2]])), dim=1
                )
            )[0]
                ...
                out_blocks.append(
                    TensorBlock(
                        samples=Labels(
                            ...
                            values=block.samples.values[idx][:, :-3],
                        ),
    ...
    return TensorMap(
        Labels(
            blocks.keys.names + ["cell_shift_a", "cell_shift_b", "cell_shift_c"],
            torch.tensor(out_block_keys),
        ),
        out_blocks,
    )
...
def move_orbitals_to_keys(in_blocks, dummy_property=None):
    ...
    keys = Labels(
        in_blocks.keys.names + ["n_i", "l_i", "n_j", "l_j"],
        torch.stack(keys).to(device=device),
    )
    # keys = block_type, species_i, species_j, L, sigma, n_i, l_i, n_j, l_j
    tmap = TensorMap(keys, blocks)
    return mts.permute_dimensions(
        tmap, axis="keys", dimensions_indexes=[0, 1, 5, 6, 2, 7, 8, 3, 4]
    )
...
def fourier_transform(H_k, T_list=None, k=None, phase=None, norm=None):
    ...
    if isinstance(H_k, torch.Tensor):
        ...
        if phase is None:
            assert (
                T_list is not None and k is not None
            ), "T_list and k must be provided when phase is None"
            phase = torch.exp(
                -2j * np.pi * torch.tensordot(k, T_list, dims=([-1], [-1]))
            )
            if len(phase.shape) == 1:
                phase = phase.reshape(1, -1)
        if norm is None:
            norm = 1 / np.sqrt(phase.shape[0])
        return norm * torch.tensordot(H_k.to(phase), phase, dims=([0], [0])).permute(
            2, 0, 1
        )  # FIXME: the normalization is incorrect when there are less Ts than kpoints (e.g., when there is a cutoff in real space)
    else:
...
def inverse_fourier_transform(H_T, T_list=None, k=None, phase=None, norm=None):
    ...
    if isinstance(H_T, np.ndarray):
        ...
        return norm * np.sum(
            [
                np.exp(2j * np.pi * np.dot(k, Ti)) * H_Ti
                for Ti, H_Ti in zip(T_list, H_T)
            ],
            axis=0,
        )
        ...
        if phase is None:
            ...
            phase = torch.exp(
                2j * np.pi * torch.tensordot(T_list, k, dims=([-1], [-1]))
            )
            if len(phase.shape) == 1:
                phase = phase.reshape(1, -1)
        if norm is None:
            norm = 1 / np.sqrt(phase.shape[0])
        return norm * torch.tensordot(H_T.to(phase), phase, dims=([0], [0])).permute(
            2, 0, 1
        )  # FIXME: the normalization is incorrect when there are less Ts than kpoints (e.g., when there is a cutoff in real space)
...
def inverse_fft(H_T, kmesh):
    ...
    if isinstance(H_T, np.ndarray):
        ...
        shape = H_T.shape[-1]
        return ifftn(H_T.reshape(*kmesh, -1), axes=(0, 1, 2), norm="ortho").reshape(
            np.prod(kmesh), shape, shape
        )
    elif isinstance(H_T, torch.Tensor):
        # return 1/np.sqrt(len(T_list))*torch.sum(torch.stack([torch.exp(2j*np.pi * torch.dot(k, Ti.type(torch.float64))) * H_Ti for Ti, H_Ti in zip(T_list, H_T)]),  dim=0)
        shape = H_T.shape[-1]
        return torch_ifftn(
            H_T.reshape(*kmesh, -1), dim=(0, 1, 2), norm="ortho"
        ).reshape(np.prod(kmesh), shape, shape)
    else:
        raise ValueError("H_T must be np.ndarray or torch.Tensor")


def kmatrix_to_blocks(
    dataset,
    device=None,
    all_pairs=False,
    cutoff=None,
    ...
    for k, block in k_target_blocks.items():
        ...
        for sample, blockval in zip(block.samples, blockval_):
            ...
            if A not in recon_Hk:
                recon_Hk[A] = torch.zeros(
                    dataset.kpts_rel[A].shape[0], norbs, norbs, dtype=torch.complex128
                )
            ...
            ipsi_jphi = slice(
                i_start + psioffset, i_start + psioffset + psi_end
            ...
            jphi_ipsi = slice(
                j_start + phioffset, j_start + phioffset + phi_end
            ...
            if abs(bt) == 1:
                ...
                raise ValueError(
                    f"bt = {bt} should not be present in kblocks_to_matrix."
                )

    for Hk in recon_Hk:
        for ik in range(len(recon_Hk[Hk])):
            assert (
                torch.norm(recon_Hk[Hk][ik] - recon_Hk[Hk][ik].conj().T) < 1e-10
            ), "Hk is not hermitian"
...
def unique_Aij_block(block):
    Aij, inv = np.unique(
        block.samples.values[:, :3].tolist(), axis=0, return_inverse=True
    )
    return Aij, inv
...
def precompute_phase(target_blocks, dataset, cutoff=np.inf):
    ...
    for k, b in target_blocks.items():
        ...
        ifrij, where_inv[kl] = unique_Aij_block(
            b
        )  # np.unique(b.samples.values[:,:3].tolist(), axis = 0, return_inverse = True)
        ...
        for I, (ifr, i, j) in enumerate(ifrij):
            ...
            phase[kl][ifr, i, j] = torch.exp(
                2j * np.pi * torch.einsum("ka,Ta->kT", kpts, Ts)
            )
...
def TMap_bloch_sums(
    ...
    for k, b in target_blocks.items():
        ...
        for I, (ifr, i, j) in enumerate(ifrij):  # enumerate(phase[kl]):
            ...
            contraction = (
                phase[kl][ifr, i, j] @ values.reshape(vshape[0], -1)
            ).reshape(pshape[0], *vshape[1:]) * factor
    ...
    if return_tensormap:
        ...
        for kl in _Hk:
            ...
            for ifr, i, j in sorted(_Hk[kl]):
                ...
                samples.extend(
                    [[ifr, i, j] + [ik] for ik in range(_Hk[kl][ifr, i, j].shape[0])]
                )
            ...
            if values != []:
                samples = Labels(
                    ["structure", "center", "neighbor", "kpoint"], torch.tensor(samples)
                )
                ...
                if is_coupled:
                    ...
                    components = [
                        Labels(
                            ["M"],
                            torch.arange(-n_M // 2 + 1, n_M // 2 + 1).reshape(-1, 1),
                        )
                    ]
                else:
                    ...
                    components = [
                        Labels(
                            ["m_i"],
                            torch.arange(-n_mi // 2 + 1, n_mi // 2 + 1).reshape(-1, 1),
                        ),
                        Labels(
                            ["m_j"],
                            torch.arange(-n_mj // 2 + 1, n_mj // 2 + 1).reshape(-1, 1),
                        ),
                    ]
        ...
        _k_target_blocks = TensorMap(
            Labels(target_blocks.keys.names, torch.tensor(keys)), _k_target_blocks
        )
...
def TMap_bloch_sums_feat(
    target_blocks, phase, indices=None, kpts_idx=None, return_tensormap=False
):
...
def blocks_to_matrix_working(
    ...
    for key, block in blocks.items():
        ...
        for sample, blockval in zip(block.samples.values, block.values):
            ...
            if T not in reconstructed_matrices[A]:
                assert (
                    mT not in reconstructed_matrices[A]
                ), "why is mT present but not T?"
                norbs = np.sum([orbs_tot[ai] for ai in dataset.structures[A].numbers])
            ...
            jphi_ipsi_slice = slice(
                j_start + phioffset, j_start + phioffset + phi_end
            ), slice(i_start + psioffset, i_start + psioffset + psi_end)

            jpsi_iphi_slice = slice(
                j_start + psioffset, j_start + psioffset + psi_end
            ), slice(i_start + phioffset, i_start + phioffset + phi_end)
    ...
    for A, matrix in enumerate(reconstructed_matrices):
        ...
        for T in Ts:
            ...
            assert torch.all(
                torch.isclose(
                    matrix[T] - reconstructed_matrices[A][mT].T,
                    torch.zeros_like(matrix[T]),
                )
...
def matrix_to_blocks_OLD(
    dataset, device=None, all_pairs=True, cutoff=None, target="fock"
):
    from mlelec.utils.metatensor_utils import TensorBuilder
    ...
    for A, frame in enumerate(dataset.structures):  # Loop over frames
        if cutoff_was_none:
            cutoff = dataset.cells[A].rcut * Bohr
            warnings.warn(
                "Automatic choice of the cutoff for structure {A}. rcut = {rcut:.2f} Angstrom"
            )
            ...
            for i, ai in enumerate(frame.numbers):
                ...
                for j, aj in enumerate(frame.numbers):
                    ...
                    if all_pairs:
                        ...
                        n1l1n2l2 = list(
                            sum([tuple(k2 + k1 for k1 in orbs_i) for k2 in orbs_j], ())
                        )
                        # print(n1l1n2l2)
                    else:
                        sorted_orbs = np.sort(
                            [
                                (o1, o2)
                                for o1, o2 in product(
                                    list(orbs_i.keys()), list(orbs_j.keys())
                                )
                            ],
                            axis=1,
                        )
                        orbs, orbital_idx = np.unique(
                            sorted_orbs, return_index=True, axis=0
                        )
