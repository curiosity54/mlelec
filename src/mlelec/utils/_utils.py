...
def _precompute_block_info(blocks, atom_blocks_idx, sort_orbs, all_pairs, orbs_offset, orbs_mult, detach):
    ...
    for key, block in blocks.items():
        ...
        blockvalues = block.values.detach().clone() if detach else block.values
...
def _initialize_matrices(reconstructed_matrices, slices_info, orbs_tot, dataset, device):
    initialized = set()
...
def _update_matrices(reconstructed_matrices, slices_info):
    for info in slices_info:
        A = info["A"]
        T = info["T"]
        mT = info["mT"]
        ...
        iphi_jpsi_slice_i, iphi_jpsi_slice_j = iphi_jpsi_slice
        ipsi_jphi_slice_i, ipsi_jphi_slice_j = ipsi_jphi_slice
        jphi_ipsi_slice_i, jphi_ipsi_slice_j = jphi_ipsi_slice
        jpsi_iphi_slice_i, jpsi_iphi_slice_j = jpsi_iphi_slice

        if block_type == 0:
            matrix_T[iphi_jpsi_slice_i, iphi_jpsi_slice_j].add_(blockval * bt0_factor_p)
            matrix_mT[jpsi_iphi_slice_i, jpsi_iphi_slice_j].add_(blockval.T * bt0_factor_m)

        elif block_type == 2:
            matrix_T[iphi_jpsi_slice_i, iphi_jpsi_slice_j].add_(blockval * bt2_factor_p)
            matrix_mT[jpsi_iphi_slice_i, jpsi_iphi_slice_j].add_(blockval.T * bt2_factor_m)

        elif abs(block_type) == 1:
            blockval.mul_(bt1_fact_fin)

            if block_type == 1:
                matrix_T[iphi_jpsi_slice_i, iphi_jpsi_slice_j].add_(blockval)
                matrix_mT[jphi_ipsi_slice_i, jphi_ipsi_slice_j].add_(blockval)
                matrix_mT[jpsi_iphi_slice_i, jpsi_iphi_slice_j].add_(blockval.T)
                matrix_T[ipsi_jphi_slice_i, ipsi_jphi_slice_j].add_(blockval.T)

            else:
                matrix_T[iphi_jpsi_slice_i, iphi_jpsi_slice_j].add_(blockval)
                matrix_mT[jphi_ipsi_slice_i, jphi_ipsi_slice_j].sub_(blockval)
                matrix_mT[jpsi_iphi_slice_i, jpsi_iphi_slice_j].add_(blockval.T)
                matrix_T[ipsi_jphi_slice_i, ipsi_jphi_slice_j].sub_(blockval.T)

def blocks_to_matrix_opt(blocks, dataset, device=None, cg=None, all_pairs=False, sort_orbs=True, detach=False, check_hermiticity=True):
    ...
    reconstructed_matrices = [dict() for _ in dataset.structures]

    slices_info = _precompute_block_info(blocks, atom_blocks_idx, sort_orbs, all_pairs, orbs_offset, orbs_mult, detach)
    _initialize_matrices(reconstructed_matrices, slices_info, orbs_tot, dataset, device)
    _update_matrices(reconstructed_matrices, slices_info)
