...
def _orbs_offsets(orbs):
    ...
    for k, orb_list in orbs.items():
        ...
        orbs_offset.update(
            {(k, n, l): offset for (n, l), offset in zip(filtered_orbs, offsets)}
        )
...
def _precompute_block_info(
    blocks, atom_blocks_idx, sort_orbs, all_pairs, orbs_offset, orbs_mult, detach
):
    slices_info = {}
    ...
    for key, block in blocks.items():
        ...
        for sample, blockval in zip(samples, blockvalues):
            ...
            bt0_factor_p = (
                0.5 if not sort_orbs else (1 if not (ni == nj and li == lj) else 0.5)
            )
            ...
            slices_info[A].append(
                {
                    "T": T,
                    "mT": mT,
                    "block_type": block_type,
                    "blockval": blockval,
                    "iphi_jpsi_slice": iphi_jpsi_slice,
                    "ipsi_jphi_slice": ipsi_jphi_slice,
                    "jphi_ipsi_slice": jphi_ipsi_slice,
                    "jpsi_iphi_slice": jpsi_iphi_slice,
...
def _initialize_matrices(
    reconstructed_matrices, slices_info, orbs_tot, dataset, device, structure_ids
):
    initialized = set()
    for iA, A in enumerate(structure_ids):
        for info in slices_info[A]:
            ...
            if (A, T) not in initialized:
                ...
                reconstructed_matrices[iA][mT] = torch.zeros(
                    norbs, norbs, device=device
                )
                initialized.add((A, T))
...
def _update_matrices(reconstructed_matrices, slices_info, structure_ids):
    for iA, A in enumerate(structure_ids):
        for info in slices_info[A]:
            ...
            info["jphi_ipsi_slice"]
            info["jpsi_iphi_slice"]
...
def blocks_to_matrix_opt(
    ...
    if slices_info is None:
        slices_info = _precompute_block_info(
            blocks,
            atom_blocks_idx,
            sort_orbs,
            all_pairs,
    ...
    for A, matrix in enumerate(reconstructed_matrices):
        ...
        for T in Ts:
            ...
            reconstructed_matrices[A][T] = (
                reconstructed_matrices[A][T] + reconstructed_matrices[A][mT].T
            )
