...
from mlelec.utils.twocenter_utils_fhiaims import (  # ,change_rotation_direction
    to_coupled_blocks,
    to_uncoupled_matrices,
)
...
def read_upper_half_realham(foldername):
    ...
    with open("{}/realspaceHam.txt".format(foldername)) as f:
        lines = f.readlines()
    for line in lines[1:]:
        icell, ibf2, ibf1, idx_real, spin, ham_ij = list(
    ...
    return (
        ham,
        idx_real,
    )  # idx_real is the number of non 0 elements in the real space hamiltonian
...
def get_nbf_ncell(foldername):
    ...
    for line in lines[1:]:
        if line.startswith(
            ...
            nbf = m[-1]
        if line.startswith(
            "  | Number of super-cells in hamiltonian [n_cells_in_hamiltonian]:"
        ):  #         104
...
def read_upper_half_realovl(foldername):
    ...
    for line in lines[1:]:
        icell, ibf2, ibf1, idx_real, ovl_ij = list(
            filter(lambda x: x != "", line.split("\n")[0].split(" "))
        )
...
def get_cellidx(foldername):
    ...
    for line in lines:
        if (
            "  | include" in line
        ):  #  | include super-cells idx: [i_cell_1]           1 newindex [i_cell_new]           1
...
def read_realhams(foldernames, suffix=""):
    ...
    for m, foldername in enumerate(foldernames[:]):
        ...
        for n, shift in enumerate(cell_locs[-1][:]):
            ...
            lo = [
                i
                for i, element in enumerate(cell_locs[-1][:])
                if element[0] == -shift[0]
                and element[1] == -shift[1]
                and element[2] == -shift[2]
            ]
            # print(lo)
            ilo.append(lo[0])
        idxup.append(iup)
    ...
    for foldername in foldernames[:]:
        rham, rham_nonzero = read_upper_half_realham(
            "{}".format(foldername)
        )  # real space Hamiltonian
...
def get_kphase(foldernames, suffix=""):
    ...
    for foldername in foldernames[:]:
        ...
        for kpt in range(1, nr_kpt + 1):
            kphase = read_kphase(
                "{}/kphase_kpt_{:06d}_scf_000000.txt".format(foldername, kpt)
            )
...
def read_Hks(foldernames, suffix=""):
    ...
    for foldername in foldernames[:]:
        ...
        for kpt in range(1, nr_kpt + 1):
            hk = read_elsi_to_csc(
                "{}/H_spin_01_kpt_{:06d}.csc".format(foldername, kpt)
            ).toarray()
            hk_list.append(hk)
...
def get_phaseshift(k, cell_locs):
    ...
    """
    return np.exp(
        2 * np.pi * 1j * np.dot(cell_locs, np.array(k).T)
    )  # np.asarray(kplist)
    ...
    for ifr in range(len(cell_i_list)):
        ...
        for icell in range(len(cell_i_list[ifr])):
            ...
            if icell < len(rh2[ifr]):
                if not (
                    abs(tuple(cell_i_list[ifr][icell])[0]) > maxshift
                    or abs(tuple(cell_i_list[ifr][icell])[1]) > maxshift
                    or abs(tuple(cell_i_list[ifr][icell])[2]) > maxshift
                ):
                    ts[tuple(cell_i_list[ifr][icell])] = np.array(rh2[ifr][icell])
        translated_matrices.append(ts)

    translated_matrices = {
        key: np.asarray([dictionary[key] for dictionary in translated_matrices])
        for key in translated_matrices[0].keys()
    }
...
def Hk_to_rH(H_k, kpoints, weights, cell):  # , kphase):
    ...
    newham = np.zeros(
        (len(H_k), len(cell[0]), len(H_k[0][0]), len(H_k[0][0][0])), dtype=np.complex128
    )
...
def check_shift_hermicity(translated_matrices, normval=10**-10):
    for T in translated_matrices.keys():
        ...
        norm = np.linalg.norm(
            translated_matrices[T]
            - np.transpose(translated_matrices[mT], axes=(0, 2, 1))
        )
        if norm > normval:
            print(T, norm)
