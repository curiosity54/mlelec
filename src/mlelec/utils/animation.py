...
from mlelec.utils.symmetry import (
...
def lissajous_euler(
    ...
    x * np.cos(tilt_angle) - z * np.sin(tilt_angle)
    x * np.sin(tilt_angle) + z * np.cos(tilt_angle)
    ...
    theta_y = np.arctan2(
        dz_tilted, np.sqrt(dx_tilted**2 + dy**2)
...
def rotated_matrices(
    init_frame, blocks, idx, orbitals, func, params=np.linspace(0, 2 * np.pi, 200)
):
    ...
    for i, rot in enumerate(ang):
        ...
        rotated_tmaps.append(mts.TensorMap(blocks.keys, r_blocks))
...
def animate_matrices(fig, axes, structures, matrices):
    def plot_frame(i):
        ...
        plot_atoms(
            structures[i], ax=ax
        )  # Assuming plot_atoms is a user-defined function
        ax.set_xlim(0, 5)
        ax.set_ylim(0, 5)
