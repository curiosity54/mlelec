...
class MLDataset:
    ...
    def _set_features(
        self, features, training_strategy, hypers_atom, hypers_pair, lcut, calc_features
    ):
        if not calc_features:
            ...
            if training_strategy == "twocenter":
                ...
                ), f"`hypers_pair` must be present when `features` is not provided and `training_strategy` is {training_strategy}."
                assert (
            ...
            if training_strategy == "twocenter":
                assert (
                    "neighbor" in features.sample_names
                ), f"Features must contain 'neighbor' label for {training_strategy}."
            elif training_strategy == "one_center":
                assert (
                    "neighbor" not in features.sample_names
                ), f"Features must not contain 'neighbor' label for {training_strategy}."
            else:
                raise NotImplementedError(
                    f"Training strategy {training_strategy} not implemented"
                )
        ...
        for flat_name in _item_names:
            ...
            if flat_name == "fockblocks":
                ...
                if self.qmdata.is_molecule:
                    ...
                    items[name] = []
                    l = self.compute_tensors(
                        [
                            torch.stack(list(h.values()))
                            for h in self.qmdata.fock_realspace
                        ]
                    )
                    items[name] = [
                        {
                            T: H[iT]
                            for iT, T in enumerate(self.qmdata.fock_realspace[ifr])
                        }
                        for ifr, H in enumerate(l)
                    ]
                ...
                if self.qmdata.is_molecule:
                    ...
                    l = self.compute_tensors(
                        [
                            torch.stack(list(h.values()))
                            for h in self.qmdata.overlap_realspace
                        ]
                    )
                    items[name] = [
                        {
                            T: H[iT]
                            for iT, T in enumerate(self.qmdata.overlap_realspace[ifr])
                        }
                        for ifr, H in enumerate(l)
                    ]
                ...
                ), "k-space Hamiltonian not available for molecules."
                items[name] = self.compute_tensors(self.qmdata.fock_kspace)

            elif flat_name == "overlapkspace":
                assert (
                    not self.qmdata.is_molecule
                ), "k-space overlap not available for molecules."
    ...
    def _split_items(self, train_frac, val_frac, test_frac):
        try:
            assert np.isclose(
                train_frac + val_frac + test_frac, 1, rtol=1e-6, atol=1e-5
            )
        except AssertionError:
    ...
    def compute_atom_resolved_density(
        ...
        for ifr, (eval, C, frame, S) in enumerate(
            zip(eigenvalues, eigenvectors, frames, overlaps)
        ):
            len(frame)
            ...
            occ = torch.tensor(
                [
                    2.0 + 0.0j if i < nelec // 2 else 0.0 + 0.0j
            ...
            if needed:
                ...
                for block in blocks_flat:
                    ...
                    squared_block = torch.nn.functional.pad(
                        block, pad_size, "constant", 0
                    )
    ...
    def _compute_model_metadata(self, qmdata=None):
        ...
        species_pair = np.unique(
            [
                comb
                for frame in qmdata.structures
        ...
        for species_counts in [
            np.unique(frame.numbers, return_counts=True) for frame in qmdata.structures
        ]:
        ...
        for k in keys:
            blocks.append(
                TensorBlock(
                    ...
                    values=torch.zeros(
                        (1, 1, 1), dtype=torch.float32, device=qmdata.device
                    ),
                )
            )
        ...
        self.model_metadata = mts.sort(
            TensorMap(
                Labels(key_names, torch.tensor(keys, device=qmdata.device)), blocks
            )
        )
        if self.orbitals_to_properties:
