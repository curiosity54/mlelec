...
def fix_gij(rho0_ij):
    ...
    if lname not in key_names:
        if "o3_lambda" in key_names:
            ...
            raise ValueError(
                "Key does not contain 'o3_lambda' or 'spherical_harmonics_l'"
            )
    ...
    for key, block in rho0_ij.items():
        try:
            ...
            try:
                ...
                raise ValueError(
                    "Key does not contain 'second_atom_type' or 'species_atom_2'"
                )
        ...
        bprops = torch.cat(
            [
                torch.from_numpy(
                    np.asarray([[neigh_species]] * len(block.properties.values))
                ),
        ...
        if L != 0:
            ...
            bvalues = torch.zeros(
                (
                    b0samples.values.shape[0],
                    block.values.shape[1],
                    block.values.shape[2],
                )
            )
            # bsam = 8
...
def sample_atom_pair(block_a, block_b, is_mp=False):
    ...
    if is_mp:
        center_slice = torch.where(
            (samples_a.values[:, None] == samples_b.values[:, [0, 2]]).all(-1)
        )[0]
        return center_slice

    else:
        neighbor_slice = torch.where(
            (samples_a.values[:, None] == samples_b.values[:, :2]).all(-1)
        )[0]
...
def cg_combine(
    ...
    if feature_names is None:
        ...
        for i in range(2, NU + 1):
            ...
            feature_names.extend(
                [f"k_{i}", f"species_neighbor_{i}", f"n_{i}", f"l_{i}"]
            )
    ...
    for index_a, block_a in x_a.items():
        ...
        for index_b, block_b in x_b.items():
            ...
            sel_feats = []
            sel_idx = []
            sel_feats = torch.cartesian_prod(
                torch.arange(len(properties_a)), torch.arange(len(properties_b))
            )  # np.indices((len(properties_a), len(properties_b))).reshape(2, -1).T

            prop_ids_a = torch.nn.functional.pad(
                properties_a.values, (0, 1), value=lam_a
            )
            prop_ids_b = torch.nn.functional.pad(
                properties_b.values, (0, 1), value=lam_b
            )

            sel_idx = torch.hstack(
    ...
    for KEY in X_blocks:
        ...
        sph_components = Labels(
            ["spherical_harmonics_m"], torch.arange(-L, L + 1).reshape(-1, 1)
        ).to(device=device)
        newblock = TensorBlock(
            values=block_data,
            samples=X_samples[KEY].to(device=device),
            components=[sph_components],
            properties=Labels(feature_names, torch.vstack(X_idx[KEY])).to(
                device=device
            ),  # torch.from_numpy(np.vstack(X_idx[KEY]))))
        )
...
def contract_rho_ij(rhoijp, elements, property_names=None):
    ...
    for key in rhoMPi_keys:
        ...
        new_block = TensorBlock(
            ...
            samples=Labels(
                ["structure", "center"],
                torch.tensor(all_block_samples, dtype=torch.int32),
            ),
            components=block.components,
            properties=Labels(
                list(property_names),
...
def compute_rhoi_pca(
    ...
    for idx, (key, block) in enumerate(rhoi.items()):
        ...
        pblock = TensorBlock(
            ...
            samples=Labels(
                ["pca"],
                torch.tensor([i for i in range(len(block.properties))]).reshape(-1, 1),
            ),
            properties=Labels(
                ["pca"],
                torch.tensor([i for i in range(vh[retained].T.shape[1])]).reshape(
                    -1, 1
                ),
            ),
        )
...
def get_pca_tmap(rhoi, pca_vh_all):
    ...
    for idx, (key, block) in enumerate(rhoi):
        ...
        pblock = TensorBlock(
            ...
            properties=Labels(
                ["pca"],
                torch.tensor([i for i in range(vt.shape[-1])]).reshape(-1, 1),
            ),
        )
        pca_blocks.append(pblock)
    pca_tmap = TensorMap(rhoi.keys, pca_blocks)
...
def apply_pca(rhoi, pca_tmap):
    ...
    for idx, (key, block) in enumerate(rhoi.items()):
        ...
        pblock = TensorBlock(
            ...
            properties=Labels(
                ["pca"],
                torch.tensor([i for i in range(xl_pca.shape[-1])]).reshape(-1, 1),
            ),
        )
