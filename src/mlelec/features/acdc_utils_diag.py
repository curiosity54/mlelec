...
import numpy as np
import torch
from metatensor import Labels, TensorBlock, TensorMap, sort_block
...
def sample_atom_pair(block_a, block_b, is_mp=False):
    ...
    if is_mp:
        ...
        while smp_b < samples_b.values.shape[0]:
            ...
            ) != (samples_a[smp_a]["structure"], samples_a[smp_a]["center"]):
...
def cg_combine(
    ...
    for index_a, block_a in x_a.items():
        ...
        for index_b, block_b in x_b.items():
            ...
            if diag_tensor:
                idx_nu1 = np.where(
                    (
                        properties_b.values[:, :2]
                        == properties_a.values[:, :2][:, None]
                    ).all(-1)
                )[1]
                sel_feats = np.array(
                    list(zip(range(len(properties_a.values)), idx_nu1))
                )
            ...
            sel_idx = np.hstack(
                [prop_ids_a[sel_feats[:, 0]][:, :], prop_ids_b[sel_feats[:, 1]][:, -1:]]
    ...
    for KEY in X_blocks:
        ...
        sph_components = Labels(
            ["spherical_harmonics_m"], np.asarray(range(-L, L + 1)).reshape(-1, 1)
        )
    ...
    if feature_names is None:
        if not diag_tensor:
            ...
            if nu == 1:
                ...
                print(
                    nu,
                    _remove_suffix(x_nu.property_names)[:2],
                    _remove_suffix(x_1.property_names),
                )
                assert _remove_suffix(x_nu.property_names)[:2] == _remove_suffix(
                    x_1.property_names
                )
...
def compute_rhoi_pca(
    ...
    for idx, (key, block) in enumerate(rhoi.items()):
        ...
        pblock = TensorBlock(
            ...
            samples=Labels(
                ["pca"],
                np.asarray([i for i in range(len(block.properties))]).reshape(-1, 1),
            ),
            properties=Labels(
                ["pca"],
                np.asarray([i for i in range(vh[retained].T.shape[1])]).reshape(-1, 1),
            ),
        )
...
def get_pca_tmap(rhoi, pca_vh_all):
    ...
    for idx, (key, block) in enumerate(rhoi):
        ...
        pblock = TensorBlock(
            ...
            properties=Labels(
                ["pca"],
                np.asarray([i for i in range(vt.shape[-1])]).reshape(-1, 1),
            ),
        )
        pca_blocks.append(pblock)
    pca_tmap = TensorMap(rhoi.keys, pca_blocks)
...
def apply_pca(rhoi, pca_tmap):
    ...
    for idx, (key, block) in enumerate(rhoi.items()):
        ...
        pblock = TensorBlock(
            ...
            properties=Labels(
                ["pca"],
                np.asarray([i for i in range(xl_pca.shape[-1])]).reshape(-1, 1),
            ),
        )
