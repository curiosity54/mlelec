    ...
    **kwargs,
    ...
    if mic : 
        ...
        for key, block in rho0_ij.items():
            samples, retained_idx = block_to_mic_translation(frames[0], block, kmesh)
            blocks.append(TensorBlock(
                values = block.values[retained_idx], 
            ...
            for smp_up in range(len(idx_up)):
                ...
                for smp_lo in range(smp_up, len(idx_lo)):
                    ...
                    if (
                        b.samples["structure"][idx_up[smp_up]]
                        != b.samples["structure"][idx_lo[smp_lo]]
                    ) or (b.samples["cell_shift_a"][idx_up[smp_up]]!= b.samples["cell_shift_a"][idx_lo[smp_lo]]) or (b.samples["cell_shift_b"][idx_up[smp_up]]!= b.samples["cell_shift_b"][idx_lo[smp_lo]]): # Must also add checks for the translation here TODO  
