...
def single_center_features(
    ...
    rhoi = calculator.compute(
        rascaline.torch.systems_to_torch(frames), use_native_system=use_native
    )
    ...
    **kwargs,
    ...
    if all_pairs:
        ...
        if hypers_pair["cutoff"] < min_cutoff:
            if overwrite_cutoff:
                ...
                warnings.warn(
                    f"Overwriting hyperparameter 'cutoff' to new value {hypers_pair['cutoff']} for all pair feature."
                )
            else:
                warnings.warn(
                    f"The selected cutoff is less than the maximum distance as repeated for kmesh ({np.ceil(min_cutoff)}) among atoms in the system!"
                )

    calculator = PairExpansion(**hypers_pair)
    rho0_ij = calculator.compute(
        rascaline.torch.systems_to_torch(frames), use_native_system=use_native
    )
    ...
    for key, block in rho0_ij.items():
        ...
        for isample, sample in enumerate(block.samples):
            ...
            if (
                False
            ):  # [i, j, x, y, z] in negative_list: # <<<<<<<< THIS MAKES HALF THE SAMPLES IN FEATURES than in targets ##FIXME pls
                continue
            else:
    ...
    rho0_ij = TensorMap(
        keys=Labels(rho0_ij.keys.names, torch.stack(keys)), blocks=blocks
    )
    ...
    if isinstance(order_nu, list):
        ...
        assert isinstance(
            order_nu, int
        ), f"order_nu = {order_nu}. Specify order_nu as int or list of 2 ints"
        order_nu_i = order_nu
        ...
        blocks.append(
            TensorBlock(
                samples=Labels(
                    ...
                    + ["neighbor", "cell_shift_a", "cell_shift_b", "cell_shift_c"],
    ...
    for k, b in pair.items():
        ...
        if k["species_center"] == k["species_neighbor"]:
            ...
            idx_ij = torch.where(
                positive_sign
                & (
                    (cell_is_zero & different_atoms)
                    | (~cell_is_zero & avoid_double_counting_atoms)
                )
            )[0]
            ...
            f_ijT = {
                1: defaultdict(
                    lambda: torch.zeros(block_values.shape[1:], device=device)
                ),
                -1: defaultdict(
                    lambda: torch.zeros(block_values.shape[1:], device=device)
                ),
    ...
    for k, b in pair.items():
        if k["species_center"] == k["species_neighbor"]:
            ...
            blocks.append(
                TensorBlock(
                    samples=Labels(
                        names=b.samples.names, values=b.samples.values[idx_up]
                    ),
                    components=b.components,
                    properties=b.properties,
...
def compute_features(
    ...
    **kwargs,
    ...
    unique_species = set.union(
        *[frame.symbols.species() for frame in dataset.structures]
    )
    structures = (
        dataset.structures
    )  # + [ase.Atoms(''.join(unique_species), positions = np.random.normal(0, 1, (len(unique_species), 3)), pbc = False)]
    ...
    hfeat = twocenter_features_periodic_NH(
        single_center=rhonui, pair=rhoij, all_pairs=all_pairs, device=device
    )
