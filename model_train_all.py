...
import xitorch 
import xitorch.linalg as xitorchlinalg
...
model = LinearTargetModel(dataset=ml_data, nlayers=0, nhidden=16, bias=False, device=DEVICE)
...
for i in range(len(molecule_data.lb_target["fock"])):
    ...
    # eig = scipy.linalg.eigvalsh(f.to('cpu'), s.to('cpu'))  
    eig,_ = xitorchlinalg.symeig(xitorch.LinearOperator.m(f), M = xitorch.LinearOperator.m(s))
    # ref_eva_lb.append(torch.from_numpy(eig).to(DEVICE))
...
for i in range(len(molecule_data.target["fock"])):
    ...
    eig,_ = xitorchlinalg.symeig(xitorch.LinearOperator.m(f), M = xitorch.LinearOperator.m(s))
    ref_eva.append(eig)#torch.from_numpy(eig))
...
#            vloss = loss_fn(data['frames'], vpred_eva, val_eva_ref)/var_eigval
    ...
    if epoch % 50 == 0:
        torch.save(model.state_dict(), f'{FOLDER_NAME}/model_output/model_epoch_{epoch}.pt')
