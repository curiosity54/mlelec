...
def read_upper_half_realham(foldername):
    ...
    with open(foldername) as f:
        lines=f.readlines()
#    ncell, nbf2, nbf1, idx_real, spin, ham =list(filter(lambda x: x != '', lines[-1].split('\n')[0].split(' ')))
#    print(ncell, nbf2, nbf2, idx_real, spin, ham)
    nbf2=0
    nbf1=0
    for line in lines[1:]:
        ncell, nibf2, nibf1, idx_real, spin, ham_ij =list(filter(lambda x: x != '', line.split('\n')[0].split(' ')))
        if int(nibf2)>nbf2:
            nbf2=int(nibf2)
        if int(nibf1)>nbf1:
            nbf1=int(nibf1)

    ham=np.zeros((int(ncell), int(nbf1), int(nbf2)))
#    print(ham.shape)
#    print(foldername)
    for line in lines[1:]:
...
def read_realhams(foldernames, suffix=''):
    ...
    for foldername in foldernames[:]:
        rham, rham_nonzero=read_upper_half_realham('{}/realspaceHam.txt'.format(foldername)) #real space Hamiltonian
...
def get_realspace_ovls(foldernames):
...
def get_kphase(foldernames, suffix=''):
    ...
    np.savez('kphase_C{}'.format(suffix), *klist)
    return klist
...
#        print(kp)
    ...
    np.savez('weights{}'.format(suffix), *wlist)
...
def rH_to_Hk(fock, kpoints,kphase,cell):
    ...
    '''
    newham=np.zeros((len(fock),len(kpoints[0]),fock[0].shape[1],fock[0].shape[2]), dtype=np.complex_)

    for ifr in range(len(fock[:1])):
        k=get_phaseshift(kpoints[ifr],cell[ifr])#.conj()
        ...
        for kpt in range(nrkpts):
            for icell in range(len(fock[ifr])):
               ...
               #     print(ifr,kpt,icell, kphase[ifr][kpt][icell],k[icell][kpt], np.linalg.norm(kphase[ifr][kpt][icell]-k[icell][kpt]))
                newham[ifr][kpt]+=fock[ifr][icell]*kphase[ifr][kpt][icell]
...
#change to condon shortley convention
    m=_fhiaims_to_condon_shortley(fock,frames,orbs)
